...
> 子集，并使用 ModelOpt 通配量化规则。Swin-T 也已使用新版脚本和 v2 builder
> 完成重测。第五、六节为当前正式结果；修正前数据仅保留为历史对照。

**汇报身份**：学生阶段性工作总结  
**实验日期**：2026-06-08（初始实验）；2026-06-13 至 2026-06-15（修正后复测）<br>
...
| 精度模式 | top1% | Δtop1（百分点） | bs=1 延迟 | bs=32 吞吐 | bs=1 加速 | 吞吐加速 |
|---|---:|---:|---:|---:|---:|---:|
| FP32 (torch) | 81.378 | — | 6.63 ms | 695 img/s | 1.00× | 1.00× |
| TRT FP16 | 81.358 | −0.020 | 1.35 ms | 4021 img/s | 4.91× | 5.79× |
| TRT INT8 v1（旧引擎） | 80.902 | −0.476 | 1.29 ms | 2523 img/s | 5.14× | 3.63× |
| **TRT INT8 v2（WeaklyTyped + FP16 + INT8 + opt5）** | **81.006** | **−0.372** | **1.25 ms** | **5358 img/s** | **5.30×** | **7.71×** |

**结论**：

- v2 相对 FP32 仅下降 **0.372 个百分点**，不需要额外 sensitive-layer
  fallback。
- v2 的 bs=32 吞吐是 TRT FP16 的 **1.33×**，是旧 v1 engine 的
  **2.12×**。
- v1 与 v2 使用相同 Q/DQ 路线，但 builder 配置不同；这组结果直接说明
  `WeaklyTyped + FP16 + INT8 + opt5` 对实际吞吐有决定性影响。
- Swin-T 是当前四个模型中最适合直接部署全量 INT8 的模型。
...
| 特性 | ViT-B | Swin-T | CNN（本次 ResNet/MobileNet） |
|---|---|---|---|
| 全量 INT8 精度损失 | 1.792 个百分点 | 0.372 个百分点 | 1.638 / 16.240 个百分点 |
| 主要敏感区域 | MLP fc1/fc2（已复测确认） | 未发现必须回退的层组 | 与具体架构有关 |
| 推荐 INT8 路径 | MLP→FP16 混合精度 | 全量 INT8 v2 | ResNet 继续调优；MobileNet 混合或 FP16 |
| per-tensor activation 适配性 | 较差 | 本次结果较好 | MobileNetV3 很差，不能概括为 CNN 都适合 |
