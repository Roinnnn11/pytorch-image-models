"""Shared utilities for the Transformer quantization workflow (ViT / Swin).

Adapted from my_optim/common.py. Centralizes the model spec, data config,
loaders, accuracy eval and latency measurement so every stage (FP32 baseline,
Torch FP16, ONNX/TRT FP16/INT8) uses an identical preprocessing pipeline —
this is what makes the cross-precision comparison fair.

Model is selected via the OPTIM_MODEL env var (default: vit_base_patch16_224).
All artifacts are written under per-model subdirectories so multiple models
can coexist:  onnx/<model>/  engines/<model>/  results/<model>/
The val/calib datasets are model-agnostic and shared at data/ (symlinked to
../my_optim/data: 50k ImageNet val + 500 calibration images).

Transformers (ViT-B, Swin-T) are heavier than CNNs, so the default eval batch
size is 32 (not 64) to stay within 12GB, and latency is reported at bs=1 and
bs=32. Override via the --batch-size flags on each stage script.
"""
import os
import sys
import time
...
MODEL_NAME = os.environ.get("OPTIM_MODEL", "vit_base_patch16_224")

# Default eval/throughput batch size for transformers (12GB-safe).
EVAL_BATCH_SIZE = int(os.environ.get("OPTIM_EVAL_BS", "32"))
# Batch sizes used for the latency sweep (bs=1 latency, bs=EVAL throughput).
LATENCY_BATCHES = (1, EVAL_BATCH_SIZE)
...
def build_calib_loader(batch_size: int = 16, workers: int = 4, cfg=None):
